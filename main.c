...
int parse_words(char *cmd, char *pcmd[]); // parses words for execvp()
...
void main_cycle()
{
	...
	while (1)
	{
		...
		synch = parse_words(cmd, pcmd); // parses words for execvp()
...
int parse_words(char *cmd, char *pcmd[])
{
	...
	{
		pcmd[i - 1] = NULL; // shows end of vector of pointers for exec()
	}
	return synch;
}
